...
| Tela Minhas Movimentações | Ótimo | Todos os campos devem sem preenchidos, mesmo se o usuário não desejar informar nenhuma observação ou a natureza da despesa.
Mesmo que o usuário coloque a data da despesa, ao adiciona-la, a mesma não é informada na tela das movimentações| Permitir que o campo 'Observações' seja opcional para preenchimento. 
O campo 'Natureza' tem a mesma função do campo 'Categoria', podendo ser removido.
Visualizar a data da despesa na página inicial da tela 'Minhas movimentações' |
